...
interface TaskSessionClientProps {
  session: Pick<TaskSession, "id" | "status" | "system_type">;
  taskDefinitions: TaskDefinition[];
  taskProgress: TaskProgress[];
  surveys: TaskSurvey[];
  surveyQuestions: TaskSurveyQuestion[];
  existingSurveyResponses: TaskSurveyResponse[];
  interviewQuestions: TaskInterviewQuestion[];
  existingInterviewResponses: UserInterviewResponse[];
...
const taskRoutes: Record<string, string> = {
  ...
  drop_course: "/registrations",
  book_room: "/facilities",
...
export function TaskSessionClient({
    ...
    const ensureProgress = async () => {
      ...
      await supabase.from("task_progress").insert(
        missing.map((definition) => ({
          ...
          status: "not_started" as TaskProgressStatus,
        })),
      );
    };
  ...
  const surveysById = useMemo(() => {
    const map: Record<string, TaskSurvey> = {};
  ...
  const questionsBySurvey = useMemo(() => {
    const grouped: Record<string, TaskSurveyQuestion[]> = {};
  ...
  const handleOpenTask = async (taskDefinitionId: string, taskCode: string) => {
    ...
    router.push(taskRoutes[taskCode] || "/");
  };
          ...
          {sortedTaskDefinitions.map((task) => {
            ...
            return (
              ...
              >
                ...
                >
                  {status === "completed" ? "Completed" : "Open Task"}
                </Button>
              </div>
            );
          })}
